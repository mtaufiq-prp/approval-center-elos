...
<div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <a href="{{ route('master.position.create') }}" class="btn btn-primary btn-sm"><i class="bi bi-plus-lg"></i> Tambah</a>
</div>
    ...
    <div class="table-responsive">
        <table class="table table-sm table-hover align-middle">
            ...
            @forelse ($items as $item)
                <tr>
                    <td><code>{{ $item->position_code }}</code></td>
                    <td>{{ $item->position_name }}</td>
                    <td class="text-muted">{{ $item->level_no }}</td>
                    ...
                    <td class="text-end">
                        <a href="{{ route('master.position.edit', $item->idtblposition) }}" class="btn btn-sm btn-outline-primary">Edit</a>
                        <form method="POST" action="{{ route('master.position.destroy', $item->idtblposition) }}"
