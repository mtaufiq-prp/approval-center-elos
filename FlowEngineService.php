...
class FlowEngineService
{
    ...
    private function enterNode(
        ...
        // APPROVAL node: BERHENTI di sini, buat task, tunggu keputusan approver
        // Jangan lanjut traverse — engine akan dilanjutkan oleh completeCurrentTask()
        if ($node->isApproval()) {
            $this->createApprovalTask($node, $instance, $request, $request->context_json ?? []);
            return;
        }

        // Node lain: lanjut traverse
    ...
    private function createApprovalTask(
        ...
        // Buat task — satu task dengan multiple candidates (mode ANY/ALL)
        // task_no: TSK-{NODE_CODE}-{REQUEST_ID}-{TIMESTAMP}
        $taskNo = 'TSK-' . strtoupper(substr($node->node_code, 0, 6))
            . '-' . $request->idtblapproval_request
            . '-' . now()->format('YmdHis');

        // Hindari duplikat task_no
        $counter = 0;
        $baseNo  = $taskNo;
        while (TblTask::where('task_no', $taskNo)->exists()) {
            $taskNo = $baseNo . '-' . (++$counter);
        }

        // Assigned ke kandidat pertama (primary), sisanya jadi candidates
        ...
        $task = TblTask::create([
            ...
            'task_no'                => $taskNo,
            'idtbluser_assigned'     => $primaryCandidate?->idtbluser,
            'task_status'            => 'OPEN',
            'due_at'                 => $node->sla_hours ? now()->addHours($node->sla_hours) : null,
        ]);

        // Semua kandidat masuk tbltas_candidate
        foreach ($candidates as $idx => $candidate) {
            TblTaskCandidate::firstOrCreate(
                ['task_id' => $task->idtbltask, 'idtbluser' => $candidate->idtbluser],
    ...
    private function completeProcess(
        ...
        $request->request_status        = $finalStatus;
        $request->completed_at           = now();
        $request->idtblflow_step_current = null; // clear current step saat selesai
        $request->save();
